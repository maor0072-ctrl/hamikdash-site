// hamikdash-nefesh - צד השרת של טופס השמות לפדיון נפש ב-hamikdash.co.il.
//
// **נבנה 2026-09-14 בהוראת יעקב**, אחרי שמסלול פדיון הכפרות עלה ועבד. זהו
// עותק מכוון של worker/kaparot.js ולא הפשטה משותפת: טופס הכפרות עומד מול
// הציבור שישה ימים לפני יום כיפור, ושינוי מבני בו כדי לחלוק קוד היה מסכן
// קמפיין חי מטעמי אלגנטיות. כשהעונה תיגמר אפשר לאחד את השניים.
//
// ההבדלים המהותיים מהכפרות, מעבר לטקסטים: פדיון נפש אינו קשור ליום כיפור
// ואינו נעשה בבית - יעקב עורך אותו בכולל ומקריא את השמות. לכן מייל האישור
// אינו מסביר איך עושים בבית, וכל שם נושא גם **בקשה** (רשות) - הישועה שעבורה
// נעשה הפדיון.
//
// המבנה, האימות מול נדרים פלוס, הטוקנים והדואר זהים לכפרות בדיוק. כל תיקון
// מהותי כאן צריך להיבדק גם שם, ולהפך.
//
const ALLOWED = [
...
const PAGE = "https://hamikdash.co.il/pidyon-nefesh/";
...
const SHEMOT_PAGE = "https://hamikdash.co.il/pidyon-nefesh/shemot.html";

// משפחה גדולה היא לגמרי סבירה כאן - פדיון נפש עושים לכל נפש, כולל ילדים וסבים.
...
async function listKey(email) {
  const buf = await crypto.subtle.digest(
    "SHA-256", new TextEncoder().encode("nefesh:" + email.toLowerCase().trim()));
  return Array.from(new Uint8Array(buf)).slice(0, 16)
    .map((b) => b.toString(16).padStart(2, "0")).join("");
}

async function readList(env, key) {
  if (!env.NEFESH) return null;
  try {
    const raw = await env.NEFESH.get("list:" + key);
...
async function writeList(env, key, data) {
  if (!env.NEFESH) return false;
  try {
    await env.NEFESH.put("list:" + key, JSON.stringify(data),
...
async function dropList(env, key) {
  if (!env.NEFESH) return false;
  try {
    await env.NEFESH.delete("list:" + key);
    return true;
  } catch (e) {
...
function confirmHtml(sender, souls, token, kind) {
  ...
  const head =
    kind === "replace"
      ...
      : `<h2 style="margin:0 0 6px">שלום ${esc(sender)}, השמות התקבלו</h2>` +
        `<p style="margin:0 0 18px;color:#4a453e">הם יעלו בפדיון הנפש שעורך הרב יעקב מאור.</p>`;
  ...
  return (
    ...
    `<h3 style="margin:0 0 8px;color:#8a6d3b">מה קורה עכשיו</h3>` +
    `<p style="margin:0 0 14px">השמות נכנסים לרשימת פדיון הנפש, ומוקראים בשמם בזמן עריכת ` +
    `הפדיון. <strong>אין מה לעשות בבית</strong> - הפדיון נערך כאן.</p>` +
    `<p style="margin:0 0 22px">אפשר להוסיף שמות בכל זמן, וגם לשנות בקשה שהשתנתה. ` +
    `הכפתורים שלמעלה פתוחים תמיד.</p>` +

    `<p style="margin:0 0 4px">בברכה,</p>` +
...
function ownerHtml(sender, phone, email, souls, note, kind) {
  const rows = souls
    ...
    .join("");
  // הבלוק להעתקה נכנס ישירות לגנרטור של מסמך פדיון הנפש, ולכן הבקשה
  // נדבקת לשם באותה שורה ובמפריד קבוע.
  const plain =
    sender + "\n" +
    souls.map((n) => fullName(n) + (n.wish ? " | " + n.wish : "")).join("\n");

  const head =
    kind === "replace"
      ? `<h2 style="margin:0 0 4px;color:#b08434">תיקון לרשימת פדיון נפש שכבר נמסרה</h2>` +
      ...
      : `<h2 style="margin:0 0 4px">פדיון נפש - ${souls.length} שמות</h2>` +
        `<p style="margin:0 0 18px;color:#4a453e">נשלח מהטופס ב-<a href="${PAGE}">עמוד פדיון נפש</a>.</p>`;
...
// פדיון נפש הוא הכסף עצמו - רשימת שמות בלי נתינה אינה פדיון. לכן הדף
...
// **הוראת יעקב 2026-09-14: תרומה קודמת אינה נחשבת.** פדיון נפש הוא נתינה
// חדשה לצורך פדיון הנפש; מי שתרם לפדיון נפש, או תרם סתם, עדיין צריך לתת סכום
...
async function kvJson(env, key, fallback) {
  try {
    const raw = await env.NEFESH.get(key);
...
async function spendNedarimCall(env) {
  ...
  calls.push(now);
  try {
    await env.NEFESH.put("ned:calls", JSON.stringify(calls), { expirationTtl: 3600 });
  } catch (e) {
...
async function refreshNedarim(env) {
  ...
  try {
    await env.NEFESH.put("ned:phones", JSON.stringify(phones));
    await env.NEFESH.put("ned:state", JSON.stringify({ cursor: cursor, fetched: Date.now() }));
  } catch (e) {
...
async function writeIntent(env, id, data) {
  try {
    await env.NEFESH.put("intent:" + id, JSON.stringify(data), {
  ...
  if (!intent.mailed) {
    try {
      const sent = await sendMail(env, {
        ...
        subject: "פדיון נפש - הדף למסירת השמות פתוח",
...
export default {
  async fetch(request, env) {
    ...
    if (request.method === "GET") {
      ...
      if (grant) {
        const payload = await verifyToken(grant, env.NEFESH_TOKEN_SECRET);
      ...
      const payload = await verifyToken(url.searchParams.get("t"), env.NEFESH_TOKEN_SECRET);
    ...
    if (d.action === "help") {
      ...
      const ok = await sendMail(env, {
        ...
        subject: "פדיון נפש - " + intent.sender + " מדווח ששילם והדף לא נפתח",
    ...
    if (d.token) {
      const payload = await verifyToken(d.token, env.NEFESH_TOKEN_SECRET);
      if (!payload) return json({ ok: false, reason: "bad token" }, 401);
      ...
      if (d.kind === "delete") {
        ...
        const ok = await sendMail(env, {
          ...
          subject: "ביטול פדיון נפש - " + prev.sender,
          html:
            `<div dir="rtl" style="font-family:Arial,sans-serif;font-size:15px;line-height:1.7">` +
      ...
      const ok = await sendMail(env, {
        to: TO, replyTo: prev.email,
        subject: (first ? "פדיון נפש - " : "תיקון פדיון נפש - ") +
      ...
      try {
        replySent = await sendMail(env, {
          to: prev.email, replyTo: REPLY_TO_OWNER,
          subject: first
            ? "פדיון נפש - השמות התקבלו, וכך עושים את הפדיון"
            : "פדיון נפש - הרשימה עודכנה",
